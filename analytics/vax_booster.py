...
class BoosterAnalytics(Linelists):
    def __init__(self):
        ...
        self.cases_two_doses, self.cases_boosted = self._get_breakthrough_cases_from_vaccinated_group()
        self.deaths_two_doses, self.deaths_boosted = self._get_breakthrough_deaths_from_vaccinated_group()
        print(" ")
    ...
    def _get_breakthrough_cases_from_vaccinated_group(self) -> [pd.DataFrame, pd.DataFrame]:
        ...
        # Breakthrough cases from double vaccinated
        is_double_jabbed = (df_cases["days_dose2"].values > 14) & (df_cases["days_dose3"].isna())
        df_cases_double = df_cases[is_double_jabbed].copy()
        df_cases_double["combo"] = df_cases_double["brand1"] + df_cases_double["brand2"]

        is_boosted = (df_cases["days_dose3"].values > 14)
        df_cases_boosted = df_cases[is_boosted].copy()
        df_cases_boosted["combo"] = (
                df_cases_boosted["brand1"] + df_cases_boosted["brand2"] + df_cases_boosted["brand3"]
        )
        return df_cases_double, df_cases_boosted

    def _get_breakthrough_deaths_from_vaccinated_group(self) -> [pd.DataFrame, pd.DataFrame]:
        ...
        is_double_jabbed = ((df_deaths["days_between_dose_2_and_positive"].values > 14) &
                            df_deaths["days_between_dose_3_and_positive"].isna())
        ...
        df_deaths_boosted = df_deaths[is_boosted].copy()
        df_deaths_boosted["combo"] = (
                df_deaths_boosted["brand1"] + df_deaths_boosted["brand2"] + df_deaths_boosted["brand3"]
        )
        return df_deaths_double, df_deaths_boosted
    ...
    def get_breakthrough_cases_summary_statistics(self, t_start: str, t_end: str) -> pd.Series:
        ...
        # Get breakthrough cases from double vaccinated group
        df_cases_two_doses = self.cases_two_doses.copy()
        is_in_time = df_cases_two_doses["date"].between(
            pd.to_datetime(t_start), pd.to_datetime(t_end)
        )
        df_cases_two_doses = df_cases_two_doses[is_in_time].copy()
        n_cases_two_doses = df_cases_two_doses["combo"].value_counts()
        ...
        n_cases = pd.concat([n_cases_two_doses, n_cases_boosted])

        combos = ["pp", "aa", "ss", "ppp", "ssp", "sss", "aaa", "aap"]
        df_breakthrough_rate = pd.Series()

        for combo in combos:
            if combo in n_cases:
                df_breakthrough_rate[combo] = n_cases[combo] / df_vax_rate[combo] * 100000
            else:
